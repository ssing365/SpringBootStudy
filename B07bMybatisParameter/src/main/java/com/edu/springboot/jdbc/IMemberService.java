 ...
 * 컨트롤러에서 인터페이스에 정의된 추상메서드를 호출하면
 * 연결된 Mapper의 특정 엘리먼트가 호출되어 실행되는 구조를 가진다.
 */
@Mapper
public interface IMemberService {
	...
	// 회원 정보 추가 
	//	: request내장객체를 통해 받은 파라미터를 개별적으로 전달
	public int insert(String id, String pass, String name);
	
	/*
	 * 회원정보 인출하기(수정1)
	 * : 파라미터를 받은 후 @Param 어노테이션으로 Mapper에서 사용할 파라미터명 지정
	 * 	  즉 id로 받은 후 _id로 변경해서 전달한다.
	 */
	public MemberDTO selectOne(@Param("_id") String id);
	
	/*
	 * 수정처리 : 파라미터를 받은 후 @Param어노테이션으로
	 * Mapper에서 사용할 파라미터명을 지정한다.
	 * 즉 id로 받은 후 _id로 변경해서 전달한다.
	 */
